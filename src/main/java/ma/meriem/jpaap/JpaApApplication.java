...
public class JpaApApplication implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        for(int j=0;j<100;j++){
            patientRepository.save(new Patient(null, "patient", new Date(), Math.random()>0.5?true:false,(int)(Math.random()*100)));
       ...
       // patientRepository.save(new Patient(null, "abdelali", new Date(), false, 100));
        ...
        List<Patient>patientList=patientRepository.chercherPatients("%p%",40);
